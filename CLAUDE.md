...
**설계 완료, 구현 미착수.** 장치 실측과 설계가 끝났다. `internal/turzx/`의 프로토콜 구현은
포팅 비용을 재려고 먼저 쓴 것이며, 구현 단계에서 코드 검토 후 적절한 위치로 옮긴다.

다음에 할 일은 `DESIGN.md` 7절 "구현 착수 순서"에 있다. 선행 조건은 사무실 Windows PC에서
인코더와 센서 소스를 확인하는 것이고, 디자인 마일스톤은 그와 병렬로 진행한다.
