...
- 라이선스 고지는 ZIP의 `NOTICES/`에 들어가고 `-SourceOffer` 없이는 패키징이 안 되게 했다. FFmpeg는 남의 GPL 빌드를 재배포하는 대신 `scripts/build-ffmpeg.ps1`로 직접 빌드하기로 정했다. 고정한 x264·FFmpeg 커밋과 이 스크립트가 곧 corresponding source의 완전한 빌드 방법이므로 제공 조건이 우리 손 안에서 해결된다. 남은 것: 빌드 산출물을 배포 payload의 `ffmpeg.exe`로 교체하고 ZIP을 다시 만들기, `-SourceOffer`에 실제 소스 제공 경로 넣기. 테마는 정리됐다 — 기본값을 `azure-ribbon`으로 바꿨고 `smon-halloween.mp4`는 Git 추적과 배포물에서 제외했다. 이미 push된 이력에는 남아 있어, 지우려면 history rewrite와 force push가 필요하다(공개 저장소였다면 이미 받아간 사본은 되돌릴 수 없다).
