...
- macOS·Linux 배포 범위. 목표는 크로스 플랫폼이고 Windows를 우선한다. 그래서
  `!windows` 분기와 gopsutil은 유지한다. 지금 보장되는 것은 컴파일까지다.
  `GOOS=linux`·`GOOS=darwin`으로 USB를 쓰지 않는 패키지가 vet을 통과한다.
  USB·센서·트레이·자동시작은 장치를 꽂고 실제로 확인해야 하며, 실제 배포 계획이
  있어야 빌드·실행·권한·서명 검증의 크기가 정해진다.
