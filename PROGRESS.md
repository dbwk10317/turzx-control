...
- 라이선스 고지는 ZIP의 `NOTICES/`에 들어가고 `-SourceOffer` 없이는 패키징이 안 되게 했다. 결정이 필요한 두 가지: (1) 동봉한 FFmpeg essentials 빌드는 GPL이므로 그 빌드의 corresponding source를 실제로 제공할 경로가 필요하다. LGPL 빌드로 바꾸거나 FFmpeg를 동봉하지 않는 선택지도 있다. (2) 테마는 정리됐다. `smon-halloween.mp4`는 개인 자산이라 Git 추적에서 빼고 배포물에도 넣지 않으며, 배포물은 `azure-ribbon`을 동봉한다. 다만 `daemon.DefaultTheme`과 AGENTS의 기본 테마 계약이 아직 `smon-halloween`이라 배포물의 기본값이 될 수 없다. 배포 기본 테마를 `azure-ribbon`으로 바꿀지 결정해야 한다. 이미 push된 이력에는 `smon-halloween.mp4`가 남아 있어, 지우려면 history rewrite와 force push가 필요하다.
