...
- 연결마다 새 `binding_id`를 발급한다. 계정 전환·연결 해제 때 이전 값과 리셋 이력을 비우고 이전 세대 파일과 늦게 완료된 조회를 거부한다. Codex는 사용량 조회 전후 `account/read`로 계정을 대조한다. Claude는 statusline payload에 계정 식별자가 없으므로 `claude auth status --json`의 `email`·`orgId`로 계정을 대조한다. 계정이 바뀌면 새 세대를 발급하고 이전 값과 리셋 이력을 비운다. 로그아웃이 확인되면 수집을 멈춘다. 바인딩이 없는 상태에서 로그인이 확인되어도 hook을 임의로 설치하지 않는다. 재연결 후에는 새 세션을 사용하고 기존 훅의 세대를 바꾸지 않는다.
