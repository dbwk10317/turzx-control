...
현재 표시 지표는 Codex·Claude의 5시간/주간 잔여량과 각 리셋까지의 시간, CPU·GPU의
사용률·온도, 그리고 RAM 사용률이다. RAM 온도는 수집하지 않는다. DIMM 열센서를 읽으려면
ring0에서 SMBus로 SPD를 접근해야 하는데 helper와 시스템 전체가 멈추는 원인이었다.
하드웨어 수집과 화면 합성은 1초 간격, 사용량 조회는 30초 간격이다.
